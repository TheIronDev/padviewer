...
module.exports = function fetchMonster(id, callback) {
	...
	request('http://puzzledragonx.com/en/monster.asp?n=' + id, function(err, resp, body) {
		...
		var regexes = [
			{name: 'name',regex: findName, size: 1},
			{name: 'type',regex: findType, size: 1},
			{name: 'element',regex: findElement, size: 1},
			{name: 'cost', regex: findCost, size: 1},
			{name: 'exp',regex: findExp, size: 1},
			{name: 'level',regex: findLevel, size: 2},
			{name: 'hp', regex: findHp, size: 2},
			{name: 'atk', regex: findAtk, size: 2},
			{name: 'rcv', regex: findRcv, size: 2},
			{name: 'sell', regex: findSell, size: 2},
			{name: 'feed', regex: findFeed, size: 2}
		];

		var result = regexes.reduce(function(memo, regexObj) {
			var result = regexObj.regex.exec(body);
			memo[regexObj.name] = result && result[1] ? result.slice(1, 1 + regexObj.size) : [''];
			return memo;
		}, {});
